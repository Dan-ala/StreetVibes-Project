...
alter table
referencia add foreign key(IdCate) references categorias(IdCate);

alter table
referencia add foreign key(IdTela) references tela(IdTela);

alter table
productos add foreign key(IdRef) references referencia(IdRef);
